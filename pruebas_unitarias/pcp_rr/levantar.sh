#!/bin/bash
# Prueba unitaria: planificacion de CORTO PLAZO -- Round Robin (RR_QUANTUM=300)
#
# Uso:
#   ./pruebas_unitarias/pcp_rr/levantar.sh
#
# El padre (PID 0) crea 3 procesos CPU-bound de ~5 segundos cada uno y termina.
# Con RR cada uno ejecuta como mucho 300 ms seguidos y despues es desalojado por
# quantum, asi que los tres se intercalan hasta terminar.
#
...
term "1 KERNEL MEMORY"    "$BASE/kernel_memory"    "./bin/kernel_memory ../pruebas_unitarias/pcp_rr/km_pcp.cfg"
sleep 2
...
sleep 2
term "4 KERNEL SCHEDULER" "$BASE/kernel_scheduler" "./bin/kernel_scheduler ../pruebas_unitarias/pcp_rr/ks_pcp.cfg test_pcp_padre"
sleep 2
...
echo ">>> Tienen que aparecer MUCHOS '## (N) - Desalojado por fin de quantum'."
echo ">>> Los 3 hijos se intercalan en vez de correr uno entero por vez."
