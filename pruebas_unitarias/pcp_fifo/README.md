# Prueba unitaria — Planificación de corto plazo: FIFO

Verifica que con `PLANIFICATION_ALGORITHM=FIFO` no exista desalojo de ningún tipo: cada
Proceso, una vez en EXEC, se queda hasta terminar.
...
El padre (PID 0) crea 3 procesos CPU-bound de ~5 s cada uno y termina.
...
```
## (1) Pasa del estado READY al estado EXEC
## (1) Pasa del estado EXEC al estado EXIT          <- sin escalas
## (1) finalizó su ejecución con motivo de SUCCESS
## (2) Pasa del estado READY al estado EXEC
## (2) Pasa del estado EXEC al estado EXIT          <- sin escalas
## (2) finalizó su ejecución con motivo de SUCCESS
## (3) Pasa del estado READY al estado EXEC
## (3) Pasa del estado EXEC al estado EXIT          <- sin escalas
## (3) finalizó su ejecución con motivo de SUCCESS
```

### Lo que hay que mirar

**1. Cero desalojos.** En toda la corrida:

```
desalojos por fin de quantum: 0
```

**2. Ningún hijo vuelve a READY.** Cada uno hace `READY -> EXEC -> EXIT` de un tirón, sin
un solo `EXEC -> READY` en el medio. Ésa es la firma de FIFO: **la única forma de dejar la
CPU es terminar o bloquearse**.

> Los `EXEC -> READY` que sí vas a ver son todos del PID 0, y no son desalojos: son la
> devolución del llamante después de cada `INIT_PROC` (`conexiones.c:362`). El proceso que
> crea a otro vuelve a la cola por su cuenta.

**3. `RR_QUANTUM` está en 100 ms y no pasa nada.** El parámetro existe en el config pero
FIFO lo ignora: `cola_usa_rr()` (`planificador.c:26`) compara contra `"RR"` y devuelve
`false`, así que `despachar()` nunca lanza el timer de quantum. Es la prueba de que el
algoritmo, y no la presencia del parámetro, es lo que manda.
...
Las prioridades son todas 0 porque en FIFO **no tienen injerencia alguna**, como dice el
enunciado. Ponerlas distintas no cambiaría nada, y eso también es verificable: cambialas y
el orden de ejecución sigue siendo el de llegada.

---

## Las configs

- **`km_pcp.cfg`**: sólo cambia `SCRIPTS_BASEPATH=../pruebas_unitarias/pcp_fifo`.
- **`ks_pcp.cfg`**: `PLANIFICATION_ALGORITHM=FIFO` (que ya es el default del repo). Todo lo
  demás queda igual, incluido `RR_QUANTUM=100`, justamente para mostrar que se ignora.
